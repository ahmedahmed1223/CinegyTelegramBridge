...
function Get-WhatsNewSections {
    ...
    return @(
        @{ Version = '8.26.6'; Items = @(
                '🔓 طبقة كانت تبقى محجوزة باسمك إلى الأبد إن تركت تجهيز قالب وضغطت زرًّا'
                '   أقدم في المحادثة — كل عرض لاحق عليها كان يُرفض للجميع حتى إعادة التشغيل.'
                '   صارت تُحرَّر لحظة انتقالك إلى شيء آخر.'
            ) }
        @{ Version = '8.26.5'; Items = @(
                '🗓 مدخلة جدولة واحدة تالفة كانت تمحو كل المواعيد القادمة نهائيًا عند'
                '   الإقلاع — صارت تُتخطّى وحدها ويبقى الباقي.'
                '🚫 المستخدم المعطَّل يبقى معطَّلًا: تلف ملف الصلاحيات كان يعيده نشطًا بصمت.'
                '⏱ مؤقّت إخفاء قديم لم يعد يُخفي غرافيكًا لا علاقة له به بعد إعادة التشغيل.'
            ) }
        @{ Version = '8.26.4'; Items = @(
                '🗂 أدوات الإدارة صارت أربع فئات (المستخدمون، المحتوى، الصحة، النظام)'
                '   بدل قائمة واحدة من خمسة عشر زرًّا — نفس الأدوات، ترتيب أوضح.'
            ) }
        @{ Version = '8.26.3'; Items = @(
                'رابط بثّ RTMP صار يُرفض إن لم يبدأ بـ rtmp:// أو rtmps://، بدل قبول أي نصّ.'
                '🩺 مركز الصحة صار يعرض عدد المحادثات المحجورة كصفّ مستقل، لا في شاشة'
                '   👥 المستخدمون وحدها.'
            ) }
        @{ Version = '8.26.2'; Items = @(
                'الجمع العربي صار صحيحًا في كل رسائل البوت تقريبًا — التذكيرات ومواعيد'
                '   الجدولة وتنبيهات شريط الأخبار وقوائم المستخدمين والتقارير: «ساعتان»'
                '   لا «2 ساعة»، و«5 دقائق» لا «5 دقيقة» دائمًا.'
                '📖 الدليل صار يذكر 🎞 جدول المواد و🤝 تسليم و✅ جاهزية المناوبة و🔇 هدوء'
                '   ساعتين، وكانت الأزرار موجودة وتعمل دون أن يذكرها الدليل.'
            ) }
