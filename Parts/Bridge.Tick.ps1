...
function Update-NewsDraftExpiry {
    ...
    if (-not $draft) { return }
    # A draft waiting for its publish time is not abandoned.
    if ($null -ne (Get-NewsPublishAt -Draft $draft)) { return }
