...
function Get-WhatsNewSections {
    ...
    return @(
        @{ Version = '6.0.0'; Items = @(
                '🧾 سجل عمليات القوالب يعرض Alias المشغّل مع معرّفه الثابت، لتتبع أوضح دون تغيير الصلاحيات.'
                '🖥️ أزرار الإعدادات أصبحت بصفوف كاملة العرض، مع إبقاء إعادة الافتراضي والتنقل واضحين.'
            ) }
        ...
        @{ Version = '6.0.0-preview.2'; Items = @(
                ...
                '🧩 مخطط موحد للإعدادات وأساس لترحيل ملفات الحالة يدعمان التوسعة القادمة.'
            ) }
        @{ Version = '6.0.0-preview.1'; Items = @(
                '⚙️ الإعدادات أصبحت أقسامًا عربية قصيرة بدل قائمة تقنية طويلة.'
                '📄 الأقسام الكبيرة موزعة على صفحات من ثمانية خيارات، مع بقاء جميع الإعدادات القديمة متاحة.'
                '🔐 أقسام الإعدادات وأزرارها ما زالت محمية بصلاحية المشرف.'
            ) }
        @{ Version = '5.7.9'; Items = @(
