...
function Get-WhatsNewSections {
    ...
    #>
    return @(
        @{ Version = '7.14.1'; Items = @(
                '➡️ أعمدة الجداول صارت تبدأ من اليمين: في شاشة الحالة المشغّل أولًا ثم القالب ثم «منذ»، ورقم الطبقة في الطرف.'
            ) }
        @{ Version = '7.14.0'; Items = @(
                '↔️ اتجاه الجداول صُحّح: الأرقام والأوقات مثل «07:10 ← 21:40» كانت تُعرض معكوسة داخل السطر العربي.'
                '📊 «الحالة الكاملة» صارت أقسامًا يُفتح منها ما تحتاجه: Cinegy · الخدمات · التشغيل · الوصول.'
            ) }
