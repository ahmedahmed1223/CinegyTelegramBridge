...
## غير مُصدَر — بعد 6.0.0

- **تحفظ سجل عمليات AIR_OP على القرص.** سجل دورة العمليات المحدود (queued/running/succeeded/warning/failed) لم يعد في الذاكرة فقط؛ يُكتب إلى `operations.json` بعد كل تغيير حالة ويُستعاد تلقائيًا عند إعادة تشغيل الجسر، بنفس أسلوب `autohide.json` و`template-reminders.json` (كتابة موثّقة مع نسخة احتياطية، وقراءة تسامحية لا توقف الإقلاع).
