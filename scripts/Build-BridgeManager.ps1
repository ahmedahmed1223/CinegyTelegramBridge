...
<#
    ...
    $script:BridgeVersion at publish time (major version only - the bridge's
    minor/patch number bumps far more often than this manager gets rebuilt,
    so tracking it exactly would just make the two look "out of sync"),
...
Write-Host "Bridge major version: $version" -ForegroundColor Cyan
