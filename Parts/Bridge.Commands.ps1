...
function Invoke-BridgeCommand {
    ...
    switch ($command) {
        ...
        { $_ -in @('مساعدة', 'help') } { Send-TelegramMessage -ChatId $ChatId -Text (Get-HelpHomeText -ChatId $ChatId -UserId $UserId) -ReplyMarkup (Get-HelpHomeKeyboard -ChatId $ChatId -UserId $UserId) }
