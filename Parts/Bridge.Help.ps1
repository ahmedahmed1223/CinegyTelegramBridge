...
function Get-HelpChaptersAr {
    ...
    $chapters = @(
        ...
        @{ Key = 'stop'; Title = '🛑 الإنهاء والطوارئ'; AdminOnly = $false; Body = @(
                ...
                ''
                '🔒 وإن كانت الطبقة محمية للمشرفين أو للمالك، فالأمر المكتوب'
                '   (/اخفاء ٩) يُرفض كما يرفضه الزرّ تمامًا.'
        ...
        @{ Key = 'time'; Title = '⏱ التوقيت والجدولة'; AdminOnly = $false; Body = @(
                ...
                ''
                '🔔 إن بقي قالب على الهواء طويلًا يصلك تنبيه بالنص المعروض'
                '   وزر «🙈 إخفاء القالب» لرفعه فورًا، وزر «✅ تمت المعالجة»'
                '   لإقراره. إن لم تؤكد تصلك متابعة واحدة بعدها.'
        ...
        @{ Key = 'urgent'; Title = '🚨 جدول العواجل'; AdminOnly = $false; Available = (Test-UrgentBoardAvailable); Body = @(
                ...
                '⏭ تخطي يعرض التالي فورًا؛ تخطي الأخير أو ⏹ إيقاف ينهي الجدول.'
                '⏸ مؤقت يبقي العاجل ظاهرًا ويجمّد الانتقال؛ ▶️ استئناف يكمله.'
                '↳ مؤقّت أمان القالب لا يتوقف. عدّل الفاصل والتكرار بالأزرار.'
                ...
                '   بالسطر التالي. 🙈 إخفاء بعد المدة يُخرج الحالي قبل التالي.'
                '⏳ «الفاصل بين الأخبار» يترك الشاشة فارغة بين خبر وآخر في وضع'
                '   exit، ويرفع أقصر فاصل مسموح بالقدر نفسه.'
                '⚙️ توقيتات الجدول؛ ولكل سطر تجاوزاته من زرّ ⚙️ بجانبه.'
            ) }
        ...
        @{ Key = 'boards'; Title = '🗂 محتوى البرامج'; AdminOnly = $false; Available = (Test-BoardsAvailable); Body = @(
                'جدول نصوص مجهَّز لبرنامج: المعدّ يكتب نصوص الحلقة مسبقًا،'
                '   والمنفّذ وقت البثّ يضغط السطر الذي يريده.'
                ''
                '📐 **كل جدول مبنيّ على قالب تختاره أنت**، والقالب هو الذي'
                '   يقرّر أمرين: <b>حقول كل صفّ</b>، و<b>الطبقة</b> التي يخرج عليها.'
                '↳ الحقول تُقرأ من المشهد نفسه، لا تُكتب ولا تُخترع.'
                '↳ فجدول «بنر الاقتصاد» على قالبٍ بحقلين = صفٌّ بحقلين لا أكثر.'
                ''
                '🆕 الإنشاء خطوتان (للمشرف): «➕ جدول جديد»'
                '↳ **الخطوة 1:** اختر القالب. كل قالب معه عدد حقوله.'
                '   ✅ صالح · ⛔ ممتنع ومعه سببه (قالبٌ لا يعلن حقلًا نصّيًا'
                '   لا يصلح لجدول، فلا يُعرض قابلًا للضغط ثم يفشل).'
                '↳ **الخطوة 2:** سمِّ الجدول كما يراه المشغّل على الزرّ.'
                '↳ والقالب **لا يتغيّر بعد الإنشاء**: لبرنامجٍ آخر جدولٌ آخر.'
                '↳ رأس شاشة الجدول يقول دائمًا قالبه وطبقته وأسماء حقوله.'
                ''
                '⚙️ إعداداته في ⚙️ الإعدادات ← 🗂 محتوى البرامج:'
                '↳ «محتوى البرامج» يفتح الباب كلّه (مطفأ افتراضيًا).'
                '↳ «أقصى عدد الجداول» و«أقصى صفوف الجدول».'
                ''
                '➕ صف جديد: املأ حقوله واحدًا واحدًا.'
                '📋 لصق دفعة: سطر لكل صف، والحقول مفصولة بـ |'
                '↳ السطر الناقص تُملأ بقيّته فارغةً، والزائد يُردّ برقمه وسببه.'
                ''
                '⬆️ ⬇️ ترتيب · ✏️ تعديل حقل · 🗑 حذف صف.'
                '🔕 تعطيل صف: يبقى نصّه محفوظًا ولا يُعرض — جهّزت عشرة'
                '   واليوم تحتاج ستة.'
                ''
                '▶️ عرض الصف · ⏹ إخفاؤه. والعرض يمرّ بالمسار المعتاد،'
                '   فحماية الطبقة والتدقيق و«ما على الهواء» كما هي.'
                ''
                '🛡 «من يملأ الجدول» (للمشرف): زرٌّ يدور بين الجميع والمشرفين'
                '   والمالك، ويعرض الحالي عليه. والعرض والإخفاء يبقيان لمن'
                '   يملك صلاحية القالب نفسه.'
                '🗑 حذف الجدول (للمشرف) بتأكيد.'
                ''
                '✂️ إن أُعيد قصّ المشهد وسقط حقل: نصّك يبقى محفوظًا ولا'
                '   يُرسَل، والشاشة تقول إن الحقل لم يعد موجودًا.'
                'نصوص فقط — الصور خارج هذا الباب عمدًا.'
            ) }
        @{ Key = 'track'; Title = '🔍 المتابعة والتحقق'; AdminOnly = $false; Body = @(
                ...
                '📡 مراقبة البثّ: يصل أو لا يصل ومنذ متى، والمصدر ودورة'
                '   الفحص، وانقطاعات آخر يوم بمُددها، ومعها زرّ اللقطة.'
        ...
        @{ Key = 'settings'; Title = '⚙️ الإعدادات'; AdminOnly = $true; Body = @(
                ...
                '🔒 وأي إعداد اسمه يشبه رمزًا أو توكن يظهر في القوائم بطوله لا'
                '   بنصّه؛ شاشته وحدها تُظهر قيمته لتنسخها.'
                ...
                ''
                '🗂 الأبواب الأحد عشر:'
                '↳ 🔐 الأمان · 🔴 الهواء · 📑 الموجز · 📚 القوالب · 📰 شريط الأخبار.'
                '↳ 🚨 جدول العواجل · 🗂 محتوى البرامج.'
                '↳ 📅 الجدولة · 📡 البث ومراقبته · 🩺 صحة Cinegy · 🗄️ الملفات والاحتفاظ · 🔔 الإشعارات.'
                '↳ 🛠️ خيارات متقدمة.'
        ...
        @{ Key = 'admin'; Title = '🛡️ أدوات المشرف'; AdminOnly = $true; Body = @(
                ...
                '🔇 هدوء ساعتين: في ⚙️ الإعدادات ← الإشعارات ← الهدوء، بجانب فترة الهدوء'
